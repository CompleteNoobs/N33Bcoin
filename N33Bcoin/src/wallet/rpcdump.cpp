...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            "importprivkey \"n33bcoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"n33bcoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CN33BcoinAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CN33BcoinAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid N33Bcoin address or script");
    }
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(CN33BcoinAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CN33BcoinAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The n33bcoin address for the private key\n"
            "\nResult:\n"
    ...
    return CN33BcoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by N33Bcoin %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CN33BcoinAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CN33BcoinSecret(key).ToString(), strTime);
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (!isScript) {
            address = CN33BcoinAddress(output);
            if (!address.IsValid()) {
        ...
        if (isP2SH) {
            ...
            CN33BcoinAddress redeemAddress = CN33BcoinAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CN33BcoinAddress pubKeyAddress = CN33BcoinAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CN33BcoinAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CN33BcoinAddress(destination);
