...
The RPM spec file provided here is for N33Bcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://n33bcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[N33Bcoin git repository](https://github.com/n33bcoin/n33bcoin) but are not part of
...
Currently only `Source10` is used, for the example `n33bcoin.conf` file.
...
in the n33bcoin git repository. At present only an SVG version of the N33Bcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the n33bcoin
...
needed when 0.12.1 is released, a proper fix is already in the N33Bcoin git
master branch.
...
idea to build N33Bcoin Core against a static version of that library compiled
...
The `n33bcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `n33bcoin-server` package. At this point, most RPM based
...
The `n33bcoin-server` package should have SELinux support. How to properly do
...
N33Bcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/n33bcoin/). He has been
packaging N33Bcoin for Fedora at least since 2012.
...
4. I renamed the `n33bcoin` package that contains the Qt GUI to `n33bcoin-core` as
that appears to be how the general population refers to it, in contrast to
`n33bcoin-xt` or `n33bcoin-classic`. I wanted to make sure the general population
