...
*Setup instructions for a Gitian build of N33Bcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the N33Bcoin
...
- [Building N33Bcoin Core](#building-n33bcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-n33bcoin-host/README.md).
...
Clone the git repositories for n33bcoin and Gitian.
...
in the n33bcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building N33Bcoin Core
----------------

To build N33Bcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the n33bcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/n33bcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/n33bcoin/n33bcoin
...
For example:
```bash
URL=https://github.com/laanwj/n33bcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit n33bcoin=${COMMIT} --url n33bcoin=${URL} ../n33bcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit n33bcoin=${COMMIT} --url n33bcoin=${URL} ../n33bcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit n33bcoin=${COMMIT} --url n33bcoin=${URL} ../n33bcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the n33bcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../n33bcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/n33bcoin
SIGPATH=/some/root/path/n33bcoin-detached-sigs

./bin/gbuild --url n33bcoin=${BTCPATH},signature=${SIGPATH} ../n33bcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/n33bcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/n33bcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/n33bcoin-osx-build.assert
```
...
[n33bcoin-core/gitian.sigs](https://github.com/n33bcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
