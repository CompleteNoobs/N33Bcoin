N33Bcoin Core version 0.11.0 is now available from:

  <https://n33bcoin.org/bin/n33bcoin-core-0.11.0/>
...
For example, add the following to `n33bcoin.conf`:
...
related to the blockchain in the n33bcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows N33Bcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that N33Bcoin Core will 
...
For an example script see [n33bcoin-submittx](https://github.com/laanwj/n33bcoin-submittx).
...
- #5334 `cf87536` libn33bcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new n33bcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only N33BcoinAmountField
- #5753 `0f44672` Add n33bcoin logo to about screen
...
- #5809 `46bfbe7` Add n33bcoin-cli man page
...
- #5849 `41259ca` contrib/init/n33bcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename N33Bcoin-Qt.app to N33Bcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for n33bcoin-tx)
